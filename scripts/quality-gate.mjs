...
async function main() {
  process.stdout.write("Quality gate dimulai.\n");
  ...
  process.stdout.write("\n=== validasi paralel: lint + test + build ===\n");
  ...
  if (failed.length > 0) {
    process.stderr.write(
      `\nQuality gate gagal: ${failed.map((item) => `${item.label}(exit ${item.code})`).join(", ")}\n`,
    );
    process.exitCode = 1;
    return;
  }

  process.stdout.write("\nQuality gate selesai: semua validasi lolos.\n");
}

main().catch((error) => {
  process.stderr.write(`quality-gate error: ${error instanceof Error ? error.message : String(error)}\n`);
  process.exitCode = 1;
});
