-- Harden cron dashboard/scheduler RPC auth.
-- Ensure only super_admin (or service_role context) can execute cron RPCs.
...
DECLARE
  ...
  v_jobs JSONB := '[]'::jsonb;
BEGIN
  IF v_uid IS NULL OR NOT is_super_admin(v_uid) THEN
