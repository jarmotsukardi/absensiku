alter table public.subscriptions
  add column if not exists billing_headcount_mode text,
  add column if not exists contracted_employee_count integer;

update public.subscriptions
set billing_headcount_mode = 'actual_active_employee'
where billing_headcount_mode is null;

alter table public.subscriptions
  alter column billing_headcount_mode set default 'actual_active_employee',
  alter column billing_headcount_mode set not null;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'subscriptions_billing_headcount_mode_check'
  ) then
    alter table public.subscriptions
      add constraint subscriptions_billing_headcount_mode_check
      check (billing_headcount_mode in ('actual_active_employee', 'manual_contract'));
  end if;
end
$$;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'subscriptions_contracted_employee_count_check'
  ) then
    alter table public.subscriptions
      add constraint subscriptions_contracted_employee_count_check
      check (contracted_employee_count is null or contracted_employee_count > 0);
  end if;
end
$$;

with latest_paid_manual_contract as (
  select distinct on (inv.tenant_id)
    inv.tenant_id,
    greatest(coalesce(inv.employee_count, 0), 1) as contracted_employee_count
  from public.invoices inv
  where inv.status = 'PAID'
    and coalesce(inv.metadata->>'billing_scope', 'centralized') <> 'individual'
    and (
      coalesce(inv.metadata->>'billing_origin', '') = 'activation_early'
      or coalesce(inv.metadata->>'employee_count_source', '') = 'manual_contract'
      or coalesce(inv.metadata->>'billing_headcount_mode_after_payment', '') = 'manual_contract'
    )
  order by inv.tenant_id, coalesce(inv.paid_at, inv.verified_at, inv.updated_at, inv.created_at) desc, inv.created_at desc, inv.id desc
)
update public.subscriptions s
set
  billing_headcount_mode = 'manual_contract',
  contracted_employee_count = l.contracted_employee_count,
  max_employees = greatest(coalesce(l.contracted_employee_count, 0), 1),
  updated_at = now()
from latest_paid_manual_contract l
where s.tenant_id = l.tenant_id
  and (
    s.billing_headcount_mode is distinct from 'manual_contract'
    or s.contracted_employee_count is distinct from l.contracted_employee_count
    or s.max_employees is distinct from l.contracted_employee_count
  );
...
declare
  ...
  v_subscription_billing_headcount_mode text := 'actual_active_employee';
  v_subscription_contracted_employee_count integer := null;
  ...
  v_notification_recipients uuid[] := '{}'::uuid[];
begin
  ...
  v_active_employee_count := greatest(coalesce(v_active_employee_count, 0), 0);
  v_employee_count := greatest(v_active_employee_count, 1);
  ...
  select
    ...
    s.intro_promo_label,
    coalesce(s.billing_headcount_mode, 'actual_active_employee'),
    s.contracted_employee_count
  into
    ...
    v_subscription_billing_headcount_mode,
    v_subscription_contracted_employee_count
  from public.subscriptions s
  where s.tenant_id = p_tenant_id
  order by s.updated_at desc nulls last, s.created_at desc nulls last
  limit 1;

  if v_subscription_billing_headcount_mode not in ('actual_active_employee', 'manual_contract') then
    v_subscription_billing_headcount_mode := 'actual_active_employee';
  end if;

  if v_subscription_billing_headcount_mode = 'manual_contract'
     and coalesce(v_subscription_contracted_employee_count, 0) > 0 then
    v_employee_count := greatest(v_subscription_contracted_employee_count, 1);
  else
    v_employee_count := greatest(v_active_employee_count, 1);
  end if;
  ...
  v_invoice_metadata := jsonb_build_object(
    ...
    'billing_headcount_mode_after_payment', v_subscription_billing_headcount_mode,
    'employee_count_source', case
      when v_subscription_billing_headcount_mode = 'manual_contract' then 'subscription_contract'
      else 'actual_active_employee'
    end,
    'contracted_employee_count_after_payment', case
      when v_subscription_billing_headcount_mode = 'manual_contract' then v_employee_count
      else null
    end,
    'active_employee_count_at_invoice', v_active_employee_count,
  ...
  select coalesce(
    array_agg(distinct ur.user_id) filter (where ur.user_id is not null),
    '{}'::uuid[]
  )
  ...
  if cardinality(v_notification_recipients) > 0 then
    v_notification_title := case
      when v_pricing_reason = 'attendance_intro_promo' then 'Tagihan Baru Streak Monitoring (Promo Onboarding)'
      else 'Tagihan Baru Streak Monitoring'
    end;
    v_notification_message := format(
      'Invoice %s untuk %s sudah terbit. Segera buka billing organisasi sebelum batas grace %s.',
      v_invoice_number,
      v_package_scope_label,
      to_char(v_due_date, 'DD Mon YYYY')
    );
    v_notification_link := '/org/billing?menu=invoices&invoice=' || v_invoice_number;
    ...
    select
      ...
      case when v_pricing_reason = 'attendance_intro_promo' then 'warning' else 'info' end,
      false,
      v_notification_link,
      jsonb_build_object(
        ...
        'reason', 'GRACE_PERIOD_ENTERED',
        'package_scope', v_package_scope,
        'package_display_name', v_package_name
      )
    from unnest(v_notification_recipients) as recipient_id;
    ...
    values (
      ...
      v_invoice_id,
      'PUSH',
      'in-app:' || cardinality(v_notification_recipients)::text,
      v_notification_title,
      v_notification_message,
      'SENT',
      now(),
      jsonb_build_object(
        'reason', 'GRACE_PERIOD_ENTERED',
        'source', 'create_pending_streak_invoice',
        'package_scope', v_package_scope,
        'pricing_reason', v_pricing_reason
      )
    );
