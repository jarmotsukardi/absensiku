CREATE OR REPLACE FUNCTION public.create_hr_evaluation_run(
  ...
  p_period_name text,
  p_period_cycle text DEFAULT NULL::text,
...
BEGIN
  ...
  SELECT *
  INTO v_existing_run
  FROM public.hr_evaluation_runs h
  WHERE h.tenant_id = p_tenant_id
    AND h.period_config_id = p_period_config_id
    AND h.status IN ('draft', 'in_review')
  ...
  ELSE
    ...
    DELETE FROM public.hr_evaluation_employee_results r
  ...
  SET
    ...
    included_employee_ids = (
      SELECT coalesce(array_agg(r.employee_id ORDER BY r.employee_id), '{}'::uuid[])
      FROM public.hr_evaluation_employee_results r
      WHERE r.run_id = v_run_id
    ),
