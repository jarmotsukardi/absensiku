-- Cron dashboard helpers + scheduler bootstrap
-- Tujuan:
-- 1) Menyediakan endpoint SQL untuk membaca semua cron task sistem.
-- 2) Menyediakan endpoint SQL untuk melihat riwayat eksekusi cron.
-- 3) Menyediakan fungsi bootstrap jadwal cron termasuk billing grace notifier.
...
BEGIN
  IF v_uid IS NOT NULL THEN
    v_allowed := is_super_admin(v_uid);
  END IF;
  ...
  -- 1) Attendance queue worker (setiap menit)
  ...
  -- 2) Cleanup GPS harian (02:00 WIB = 19:00 UTC)
  ...
  -- 3) Analyze partisi harian (03:00 WIB = 20:00 UTC)
  ...
  -- 4) Cleanup audit mingguan (Minggu 03:00 WIB = Sabtu 20:00 UTC)
  ...
  -- 5) Create partition bulanan (tgl 25 jam 01:00 WIB = 24 18:00 UTC)
  ...
  -- 6) Sinkron status subscription vs streak (00:10 WIB = 17:10 UTC)
  ...
  -- 7) Notifier invoice grace period (HTTP ke Edge Function)
