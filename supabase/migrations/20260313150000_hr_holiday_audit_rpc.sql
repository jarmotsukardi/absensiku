CREATE OR REPLACE FUNCTION public.get_hr_holiday_audit_summary(
  p_tenant_id uuid DEFAULT NULL
)
RETURNS TABLE (
  total_holidays bigint,
  global_mismatch_count bigint,
  tenant_marked_national_count bigint,
  duplicate_holiday_count bigint
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  WITH scoped_holidays AS (
    SELECT h.id, h.date, h.name, h.tenant_id, h.is_national
  ...
  SELECT
    count(*)::bigint AS total_holidays,
    count(*) FILTER (
      WHERE tenant_id IS NULL AND (is_national = false OR is_national IS NULL)
    )::bigint AS global_mismatch_count,
    count(*) FILTER (
      WHERE tenant_id IS NOT NULL AND is_national = true
    )::bigint AS tenant_marked_national_count,
    COALESCE((SELECT sum(duplicate_count) FROM duplicate_groups), 0)::bigint AS duplicate_holiday_count
  FROM scoped_holidays;
END;
$$;
...
GRANT EXECUTE ON FUNCTION public.get_hr_holiday_audit_summary(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_hr_holiday_audit_findings(uuid, integer, integer) TO authenticated;
