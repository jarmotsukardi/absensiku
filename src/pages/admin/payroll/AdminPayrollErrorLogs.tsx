...
import { Copy, Download, Eye, RefreshCcw, Search, Trash2 } from "lucide-react";
...
const inferContext = (row: PayrollAuditLog): string => {
  ...
  if (route.includes("/roles")) return "admin.payroll.roles";
  if (route.includes("/validation")) return "admin.payroll.validation";
  if (route.includes("/run-engine")) return "admin.payroll.run_engine";
...
const inferRoute = (row: PayrollAuditLog): string => {
  ...
  if (row.entity_type === "payroll_webhook") return "/admin/payroll/integrations";
  return "/admin/payroll/audit";
};
...
export default function AdminPayrollErrorLogs() {
  ...
  const handleExportCsv = () => {
    const csv = [
      ["ref_error", "waktu", "tenant_id", "konteks", "pesan", "route", "severity", "status", "actor_role", "notes"],
    ...
    anchor.href = url;
    anchor.download = `admin-payroll-error-log-${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  const handleExportJson = () => {
    ...
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = `admin-payroll-error-log-${new Date().toISOString().slice(0, 10)}.json`;
    document.body.append(anchor);
    anchor.click();
    anchor.remove();
    URL.revokeObjectURL(url);
  ...
  return (
    <SuperAdminLayout title="Log Error Payroll" subtitle="Triase error lintas tenant" workspaceMode="payroll">
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="space-y-2">
            <Badge variant="destructive">Log Error</Badge>
            <h1 className="text-2xl font-semibold tracking-tight">Log Error Payroll (Superadmin)</h1>
            <p className="text-sm text-muted-foreground">
              Agregasi error/gagal payroll lintas tenant berdasarkan nomor referensi.
            </p>
          </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {summary.total} error tercatat. Rentang {timeRange === "24h" ? "24 jam" : timeRange === "7d" ? "7 hari" : "30 hari"}.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="grid gap-3 md:grid-cols-5">
              <div className="relative md:col-span-2">
                ...
                <Input
                  className="pl-9"
                  placeholder="Cari berdasarkan ref, tenant, konteks, pesan, route..."
                  value={searchTerm}
            ...
            <div className="grid gap-3 md:grid-cols-3">
              ...
              <div className="flex items-center justify-between rounded-md border px-3 py-2 text-sm">
                <span className="text-xs text-muted-foreground">
                  Auto-refresh dinonaktifkan untuk menjaga performa gabungan absensi + HR + payroll.
                </span>
              </div>
            </div>
          </CardContent>
