...
import { getOrgHrPageGuide } from "@/lib/orgHrPageGuide";

type OrgHRPageGuideProps = {
  pathname: string;
};

export function OrgHRPageGuide({ pathname }: OrgHRPageGuideProps) {
  const guide = getOrgHrPageGuide(pathname);
  ...
  return (
    <Card className="border-primary/20 bg-primary/5">
      ...
      <CardContent className="space-y-4">
        ...
        {guide.watchouts?.length ? (
          <GuideListCard title="Hal yang Perlu Diwaspadai" items={guide.watchouts} />
        ) : null}
